...
```
A classical one, supports `N` ([nulla](https://en.wikipedia.org/wiki/Roman_numerals#Zero)) for zero. Converts values to int in range from 0 to 3999 (inclusively). Performs input validation. E.g., inputs like `IIII`, `IIX` or `MMMMMXX` are considered incorrect.
```java
...
```
A classical one. Permits zero (the result would be `N` - nulla). The value of `number` should lie in range [ 0 .. 3999 ]. If the argument `shortest` is set to true then the shortest possible output is generated (but it won't be a classical one). Example:

`toRomanNumeralsString( 49, false )` = `XLIX` - correct classical way;
